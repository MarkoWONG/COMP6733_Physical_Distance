...
// BLE Service Name
BLEService pingingService("180C");

// BLE Characteristics
// Syntax: BLE<DATATYPE>Characteristic <NAME>(<UUID>, <PROPERTIES>, <DATA LENGTH>)
BLEStringCharacteristic ble_ping("2A56", BLERead | BLEWrite, 13);
...
void setup() {
  ...
  BLE.begin();
  BLE.setLocalName("Marko Nano 1");
  // Setting BLE Service Advertisment
  BLE.setAdvertisedService(pingingService);
  
  // Adding characteristics to BLE Service Advertisment
  pingingService.addCharacteristic(ble_ping);

  // Show it self to the world
  BLE.advertise();
  ...
  // BLECharacteristic readCharacterisic = peripheral.characteristic("2a19");
